...
const TeacherSidebar = ({ isOpen, onClose, isCollapsed, onToggleCollapse }: SidebarProps) => {
  ...
  const sidebarItems = [
    { name: "Dashboard", icon: <LayoutDashboard size={20} />, path: "/teacherdashboard" },
    { name: "Upload Video", icon: <Video size={20} />, path: "/teacher/upload-video" },
    { name: "Upload Notes", icon: <FileText size={20} />, path: "/teacher/upload-notes" },
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!isCollapsed && (
              <div className="transition-opacity duration-300">
                <h2 className="text-lg font-bold text-red-600 leading-tight whitespace-nowrap">Teacher Panel</h2>
            ...
            <div className={`flex items-center ${isCollapsed ? "justify-center" : "gap-2"}`}>
              <div className="w-8 h-8 bg-red-100 rounded-full flex items-center justify-center overflow-hidden border border-red-200 shrink-0">
                <img
                  src={userData?.profile_image
                    ? (userData.profile_image.startsWith('http') ? userData.profile_image : `http://localhost:8000${userData.profile_image}`)
                    : `https://api.dicebear.com/7.x/avataaars/svg?seed=${userData?.username || "Teacher"}&backgroundColor=b6e3f4`}
                  alt="Avatar"
              ...
              {!isCollapsed && (
                <div className="flex-1 min-w-0 transition-opacity duration-300">
                  <p className="text-sm font-bold text-gray-800 truncate">{userData?.name || userData?.username || "Teacher"}</p>
                  <p className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Instructor</p>
                </div>
              )}
            </div>
          </div>
